...
+ (DSActivityView *)newActivityViewForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
    ...
    if (dsActivityView) return dsActivityView;
...
- (DSActivityView *)initForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
    ...
    [self addSubview:self.borderView];
     ...
     NSURL *url=[[NSURL alloc] initFileURLWithPath:[[NSBundle mainBundle] pathForResource:@"loader" ofType:@"gif"]];

    
    UIImage* mygif = [UIImage animatedImageWithAnimatedGIFURL:url];
    [self.webView setImage:mygif];
    
   // NSData * gifData = [NSData dataWithContentsOfFile:];
...
//    [self.webView setOpaque:NO];
  ...
  //  [self.webView setDelegate:self];
    [self.borderView addSubview:self.webView];
    [self.borderView addSubview:self.activityLabel];

    if([self.activityLabel.text isEqualToString:@"Loading all jobsites..."])
    {
        UILabel *lblMsg = [[UILabel alloc] initWithFrame:CGRectMake(10, 380, 300, 30)];
        lblMsg.text = @"Double tap to cancel the service.";
        lblMsg.font = [UIFont fontWithName:@"Helvetica" size:14.0];
        lblMsg.textAlignment = NSTextAlignmentCenter;
        lblMsg.layer.cornerRadius =   10.0;
...
//{
//    NSLog(@"Localized description of error is %@",error.localizedDescription);
//
//}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([[touches anyObject] tapCount] == 2)
    {
        if([self.activityLabel.text isEqualToString:@"Loading all jobsites..."])
        {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"AllJSParserCancel" object:nil];
...
- (UIImageView *)webView
{
    if (!_webView)
    {
        _webView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 50, 50)];
        //[_webView setCenter:self.center];
    }
...
- (void)layoutSubviews;
{
    ...
    borderFrame.size.width = self.webView.frame.size.width + textSize.width + 25.0;
    borderFrame.size.height = self.webView.frame.size.height + 10.0;
    ...
    CGRect indicatorFrame = self.webView.frame;
	indicatorFrame.origin.x = 10.0;
	indicatorFrame.origin.y = 0.5 * (borderFrame.size.height - indicatorFrame.size.height);
    self.webView.frame = indicatorFrame;
...
- (UILabel *)activityLabel;
{
    if (!_activityLabel)
    {
        ...
        _activityLabel.shadowColor = [UIColor grayColor];
    }
...
- (UIView *)borderView;
{
    if (!_borderView)
    {
       ...
       // _borderView.backgroundColor = [[UIColor clearColor] colorWithAlphaComponent:0.5];
...
- (void)layoutSubviews;
{
    ...
    if (textSize.width < self.webView.frame.size.width)
        textSize.width = self.webView.frame.size.width + 10.0;
    ...
    borderFrame.size.height = self.webView.frame.size.height + textSize.height + 40.0;
    borderFrame.origin.x = floor(0.5 * (self.frame.size.width - borderFrame.size.width));
